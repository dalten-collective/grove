...
import { createState, createSubscription, pokeOptimisticallyN, reduceStateN, } from './base';
import api from '../api';
...
export const useSettingsState = createState('Settings', (set, get) => ({
    display: {
        theme: 'auto',
    },
    calmEngine: {
    ...
    fetchAll: async () => {
        const grResult = (await api.scry(getDeskSettings(window.desk)))
            .desk;
        const lsResult = (await api.scry(getDeskSettings(lsDesk))).desk;
        const newState = {
            ..._.mergeWith(get(), grResult, lsResult, (obj, src) => _.isArray(src) ? src : undefined),
            loaded: true,
        };
        set(newState);
    },
}), ['display', 'heaps', 'diary', 'groups', 'talk'], [
    (set, get) => createSubscription('settings-store', `/desk/${window.desk}`, (e) => {
    ...
    (set, get) => createSubscription('settings-store', `/desk/${lsDesk}`, (e) => {
        const data = _.get(e, 'settings-event', false);
        if (data) {
            reduceStateN(get(), data, reduceUpdate);
            set({ loaded: true });
