...
export function sampleQuippers(quips) {
    return _.flow(f.map(([, q]) => q.memo.author), f.compact, f.uniq, f.take(3))([...quips]);
}
...
export function makePrettyDayAndDateAndTime(date) {
    const diff = differenceInDays(endOfToday(), date);
    const time = makePrettyTime(date);
    const fullDate = `${format(date, 'LLLL')} ${format(date, 'do')}, ${format(date, 'yyyy')}`;
    switch (true) {
        case diff === 0:
...
export function whomIsFlag(whom) {
    return (/^~[a-z-]+\/[a-z]+[a-z0-9-]*$/.test(whom) &&
...
export const IMAGE_REGEX = /(\.jpg|\.img|\.png|\.gif|\.tiff|\.jpeg|\.webp|\.webm|\.svg)$/i;
export const AUDIO_REGEX = /(\.mp3|\.wav|\.ogg|\.m4a)$/i;
export const VIDEO_REGEX = /(\.mov|\.mp4|\.ogv)$/i;
export const URL_REGEX = /(https?:\/\/[^\s]+)/i;
export const PATP_REGEX = /(~[a-z0-9-]+)/i;
export const IMAGE_URL_REGEX = /(http(s?):)([/|.|\w|\s|-]|%2*)*\.(?:jpg|img|png|gif|tiff|jpeg|webp|webm|svg)/i;
...
const isFacebookGraphDependent = (url) => {
    ...
    return (caseDesensitizedURL.includes('facebook.com') ||
...
export function canReadChannel(channel, vessel) {
...
export function isChannelImported(nest, pending) {
    ...
    return (!isImport || (isImport && pending[nest]) || window.our === getNestShip(nest));
}
